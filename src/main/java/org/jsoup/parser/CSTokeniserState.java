...
/**
 * States and transition activations for the CSTokeniser.
 */
public enum CSTokeniserState {
	Data {
		// in data state, gather characters until a character reference or tag is found
		void read(CSTokeniser t, CharacterReader r) {
			switch (r.current()) {
				case '&':
	...
	RCDATAEndTagName {
		...
		private void anythingElse(CSTokeniser t, CharacterReader r) {
	...
	abstract void read(CSTokeniser t, CharacterReader r);
	...
	private static final char replacementChar = CSTokeniser.replacementChar;
	private static final String replacementStr = String.valueOf(CSTokeniser.replacementChar);
	 ...
	 */
	private static void handleDataEndTag(CSTokeniser t, CharacterReader r, CSTokeniserState elseTransition) {
		if (r.matchesLetter()) {
			String name = r.consumeLetterSequence();
			t.tagPending.appendTagName(name);
			t.dataBuffer.append(name);
	...
	private static void handleDataDoubleEscapeTag(CSTokeniser t, CharacterReader r, CSTokeniserState primary, CSTokeniserState fallback) {
		if (r.matchesLetter()) {
			String name = r.consumeLetterSequence();
			t.dataBuffer.append(name);
			t.emit(name);
